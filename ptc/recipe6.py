...
class RomanNumeralsConverterTest(unittest.TestCase):
    def setUp(self):
...
def high_and_low():
    ...
    suite.addTest(RomanNumeralsConverterTest("test_parsing_millenia"))
    suite.addTest(RomanNumeralsConverterTest("test_parsing_one"))
    return suite

def combos():
    return unittest.TestSuite(map(RomanNumeralsConverterTest, \
                                  ["test_combo1", "test_combo2", "test_combo3"]))

def all():
    return unittest.TestLoader().loadTestsFromTestCase(RomanNumeralsConverterTest)

if __name__ == "__main__":
    for suite_func in [high_and_low, combos, all]:
        print(suite_func.__name__)
        suite = suite_func()
